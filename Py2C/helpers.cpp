...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	//Nested for loop to iterate through grid and each element into total for normalization
	for(i = 0; i < rows; i++) {
		for(j = 0; j < cols; j++) {
			total += grid[i][j];
		}
	...
	//Nested for loop to iterate through grid and normalize each element
	for(k = 0; k < rows; k++) {
		for(l = 0; l < cols; l++) {
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	//Create window matrix using C++11 syntax
