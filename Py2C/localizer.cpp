...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  //Capture rows and colums of input grid
  int height = grid.size();
  int width = grid[0].size();

  //Initialize new grid to same size as input grid with zeroes in all spots
  vector< vector<float> > newGrid;

  //Calculate area of grid and initial probability per cell
  float area = float(height*width);
  float belief_per_cell = 1.0 / area;

  int i, j;

  //Initialize vector for a single row
  vector<float> singlerow;

  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      //Add element to row vector
      singlerow.push_back(belief_per_cell);
    }
    //Add entire row that was just made to new grid
    newGrid.push_back(singlerow);
    //Make the row vector empty for next iteration of the for loop
    singlerow.clear();
  }
  ...
  float blurring) 
{
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      //Use conversion from Python modulo to C found on Stack Overflow
      // ((a % b) + b) % b in C == a % b in Python  
      int new_i = ((((i + dy) % height) + height) % height);
      int new_j = ((((j + dx) % width) + width) % width);
      //Update cell of newGrid 
	...
	float p_miss) 
{
  ...
  //Initialize vector for a single row
  vector<float> singlerow;
  //Initialize varable to keep sum of all elements in grid
  float summation = 0.0;
  ...
  for (i = 0; i < height; i++) {
    for (j = 0; j < width; j++) {
      //Calculate if the sensed color is the same as the color in the cell
      bool hit = (color == grid[i][j]);
      //Add element to row vector
      singlerow.push_back(beliefs[i][j] * ((hit*p_hit) + ((1-hit)*p_miss)));
    }
    //Add entire row that was just made to new grid
    newGrid.push_back(singlerow);
    //Make the row vector empty for next iteration of the for loop
